  ...
  try {
    ...
    if (!station) {
      await auditFeriaEvent(supabase, {
        session,
        action: "FERIA_PROGRESS_DENIED",
        description: "Intento de avance en estación inexistente.",
    ...
    const puntosGanados = getProgressPoints(station as JsonRecord);
    const { data, error } = await supabase.rpc("internal_feria_registrar_progreso", {
      p_session_id: session.id,
      p_estudiante_id: session.estudiante_id,
      p_estacion_id: estacionId,
      p_puntos_ganados: puntosGanados,
      p_request_id: requestId,
    });

    if (error) throw error;

    return jsonResponse({ progress: data }, 200, corsHeaders);
  } catch (error) {
