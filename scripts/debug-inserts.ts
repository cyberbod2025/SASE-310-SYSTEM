...
async function main() {
  const testEmail = process.env.TEST_DEBUG_EMAIL || "debug.inserts@sase.mx";
  const testPassword = process.env.TEST_DEBUG_PASSWORD || (() => { throw new Error("Falta TEST_DEBUG_PASSWORD en .env.local"); })();
