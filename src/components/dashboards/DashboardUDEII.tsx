...
export const DashboardUDEII = () => {
  const { students, addIncident, updateBapInfo } = useApp();
  ...
  const handleSaveAdjustment = async (data: any) => {
    ...
    const updatedBap: any = {
      ...student.bapInfo,
      accommodations: [...student.bapInfo.accommodations, data.adjustment],
    };

    await updateBapInfo(student.id, updatedBap);

    // Also log as an academic incident for visibility
    await addIncident(
      student.id,
      IncidentType.ACADEMICO,
      `🔧 AJUSTE RAZONABLE AÑADIDO: ${data.adjustment} (${data.category})`,
    );
    ...
    <div className="flex-1 w-full space-y-8 animate-fade-in relative z-10 transition-all">
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={() =>
              toast("Funcionalidad de Registro en desarrollo", { icon: "📝" })
            }
