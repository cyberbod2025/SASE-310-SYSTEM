                      ...
                      recentVisits.map((visit, idx) => (
                        ...
                        >
                          <td className="px-8 py-6">
                            <span className="text-sase-clinical font-mono text-xs tabular-nums tracking-tighter bg-sase-clinical/5 px-3 py-1.5 rounded-2xl border border-sase-clinical/10">
                          ...
                          <td className="px-8 py-6 text-right">
                            <button className="size-10 rounded-2xl bg-slate-100 border border-slate-200 text-slate-700 flex items-center justify-center hover:bg-sase-clinical/20 hover:text-sase-clinical hover:border-sase-clinical/40 transition-all active:scale-90">
        ...
        <div className="space-y-8">
          ...
          <div className="card-sase p-8 border-sase-danger/20 bg-sase-danger/[0.03] relative overflow-hidden group">
            <div className="flex items-center gap-5 mb-8 relative z-10">
              <div className="size-14 bg-sase-danger/10 border border-sase-danger/30 rounded-2xl flex items-center justify-center text-sase-danger">
            ...
            <div className="space-y-3 relative z-10">
              <button
                ...
                disabled={notifying}
                className="w-full py-4 bg-sase-danger text-white font-black text-[11px] uppercase tracking-[0.2em] rounded-2xl hover:bg-sase-danger transition-all shadow-[0_0_30px_rgba(244,63,94,0.3)] flex items-center justify-center gap-4 disabled:opacity-50"
              >
                {notifying ? (
...
const VitalMonitor = ({ label, value, unit, icon, color, points }: any) => (
  <div className="card-sase p-6 border-sase-clinical/10 bg-[#0a0f18]/40 backdrop-blur-xl relative overflow-hidden group">
    ...
    <div className="flex items-end gap-3 mb-6 relative z-10">
      ...
      <span className="text-xs font-black text-sase-clinical uppercase tracking-widest mb-2 italic">
        {unit}
      </span>
    </div>
    <div className="h-12 w-full flex items-end gap-1 px-1 opacity-60">
      {points.map((h: number, i: number) => (
        <motion.div
          ...
          className={`flex-1 rounded-t-sm bg-sase-clinical/30`}
        />
      ))}
    </div>
  </div>
        ...
        return (
          <div key={item.id} className="group/item">
            ...
            <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
              <motion.div
                animate={{ width: `${pct}%` }}
                className={`h-full ${pct < 30 ? "bg-sase-danger" : "bg-sase-clinical"}`}
              />
            </div>
          </div>
        );
