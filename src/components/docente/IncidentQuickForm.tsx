...
export const IncidentQuickForm: React.FC<IncidentQuickFormProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[9998] flex items-end justify-center bg-slate-950/70 p-3 backdrop-blur-sm sm:items-center">
      <section className="w-full max-w-2xl rounded-[2rem] border border-emerald-200/20 bg-slate-950 p-5 shadow-2xl shadow-emerald-950/40">
        <div className="mb-5 flex items-start justify-between gap-3">
          ...
          <button type="button" onClick={onClose} className="rounded-2xl border border-white/10 bg-white/[0.06] px-3 py-2 text-[10px] font-black uppercase tracking-widest text-white">
            Cerrar
          </button>
        </div>
        ...
        <div className="space-y-4">
          <label className="block">
            ...
            <select
              ...
              className="min-h-[48px] w-full rounded-2xl border border-white/10 bg-slate-900 px-4 text-sm font-bold text-white outline-none focus:border-emerald-300/50"
            >
              ...
              {QUICK_INCIDENT_OPTIONS.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={`min-h-[82px] rounded-2xl border p-3 text-left transition ${active ? "border-emerald-300 bg-emerald-500/20" : "border-white/10 bg-white/[0.05] hover:border-emerald-300/40"}`}
                  >
          ...
          <label className="block">
            ...
            <textarea
              ...
              className="min-h-[82px] w-full resize-none rounded-2xl border border-white/10 bg-slate-900 p-4 text-sm text-white outline-none placeholder:text-slate-600 focus:border-emerald-300/50"
            />
          </label>
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            <label className="block">
              ...
              <input
                ...
                className="min-h-[44px] w-full rounded-2xl border border-white/10 bg-slate-900 px-4 text-sm text-white outline-none placeholder:text-slate-600 focus:border-emerald-300/50"
              />
            </label>
            <label className="block">
              ...
              <input
                ...
                className="min-h-[44px] w-full rounded-2xl border border-white/10 bg-slate-900 px-4 py-2 text-xs text-slate-300 file:mr-3 file:rounded-xl file:border-0 file:bg-emerald-300 file:px-3 file:py-2 file:text-[10px] file:font-black file:uppercase file:tracking-widest file:text-slate-950"
              />
            </label>
          </div>
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            <button type="button" onClick={() => onSubmit(false)} disabled={!canRegister} className="min-h-[52px] rounded-2xl bg-emerald-300 px-4 text-sm font-black uppercase tracking-widest text-slate-950 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400">
              Guardar
            </button>
            <button type="button" onClick={() => onSubmit(true)} disabled={!canRegister} className="min-h-[52px] rounded-2xl border border-emerald-300/30 bg-emerald-500/10 px-4 text-sm font-black uppercase tracking-widest text-emerald-100 disabled:cursor-not-allowed disabled:opacity-40">
              Guardar y continuar
            </button>
          </div>
        </div>
