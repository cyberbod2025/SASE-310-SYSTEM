...
2.  **Dev/Soporte (Cuenta Técnica)**: Rol `DEVELOPER`. **Debe ser un correo distinto al institucional** (ej. personal o de soporte). Se usa solo para mantenimiento, respaldos y emergencias. No para clases.
3.  **Tú (Hugo Sánchez)**: Rol `DOCENTE` + `TUTOR`. Tu cuenta operativa diaria para clases y grupos. Ya configurada.
...
2.  Llenan sus datos (Nombre, CURP, Rol Solicitado, Turno).
3.  El sistema crea una `Solicitud de Alta` en estado `PENDIENTE`.

### Paso B: Aprobación (El "Visto Bueno")

1.  El **Director (o Tú/Dev)** inicia sesión.
2.  En el **Dashboard Dirección**, verán una notificación o un botón **"Aprobaciones"**.
3.  Entran al módulo **Aprobaciones de Personal**.
4.  Revisan la lista de solicitantes.
5.  Al dar click en **"Aprobar"**:
...
2.  Usan la función **"Carga Masiva e IA"** que verificamos anteriormente.
3.  Suben las listas de Excel de los grupos.
4.  El sistema puebla la tabla `alumnos`.
